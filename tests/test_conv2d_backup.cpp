...
// ================================
// Fixed Training Test - Gradient Explosion Fix
// ================================

TEST(mnist_training_fixed, stable_training) {
    /*
    Fixed training with much lower learning rate
    to prevent gradient explosion
    */
    
    std::cout << "\n🔧 Testing Fixed Training (Lower LR)..." << std::endl;
    
    // Create SMALLER CNN for stability
    auto stable_cnn = net::layer::Sequence(
        net::layer::Conv2d(1, 16, 3, 1, 1),    // Smaller: 16 filters instead of 32
        net::layer::ReLU(),
        net::layer::MaxPool2d(2),
        net::layer::Conv2d(16, 32, 3, 1, 1),   // Smaller: 32 filters instead of 64
        net::layer::ReLU(), 
        net::layer::MaxPool2d(2),
        net::layer::Flatten(),
        net::layer::Linear(1568, 128),          // Smaller: 128 instead of 512
        net::layer::ReLU(),
        net::layer::Linear(128, 10)
    );
    
    // MUCH SMALLER Learning Rate
    auto optimizer = std::make_shared<net::optimizer::SGD>(0.0001f);  // 0.01 → 0.00001 (100x smaller!)
    stable_cnn.configure_optimizer(optimizer);
    
    // Load MNIST data
    net::Dataset mnist_data(16, false);  // Smaller batch: 32 → 16
    std::string data_dir = "../data/mnist/";
    ...
    try {
        ...
        for (int batch_idx = 0; batch_idx < num_batches; ++batch_idx) {
            ...
            for (int i = 0; i < 16 * 784; ++i) {
                cnn_data_ptr[i] = mnist_data_ptr[i];
            }
            ...
            // Check for exploding values BEFORE loss calculation
            float* output_data = output.data();
            ...
            if (max_output > 1000.0f || min_output < -1000.0f) {
                std::cout << "⚠️  Large values detected - Max: " << max_output << ", Min: " << min_output << std::endl;
            }
            ...
            if (batch_loss > 100.0f) {
                std::cout << "⚠️  Very high loss: " << batch_loss << std::endl;
            }
            ...
            std::cout << "  Batch " << (batch_idx + 1) << "/" << num_batches 
                     ...
                     << ", Max=" << std::setprecision(1) << max_output << std::endl;
        }
        ...
        std::cout << "\n🎯 Stable Training Results:" << std::endl;
        std::cout << "📈 Average Loss: " << std::fixed << std::setprecision(4) << avg_loss << std::endl;
        std::cout << "🎯 Final Accuracy: " << std::setprecision(2) << final_accuracy << "%" << std::endl;
        std::cout << "⏱️  Total Training Time: " << total_training_time.count() << " ms" << std::endl;
        ...
        EXPECT_GT(final_accuracy, 5.0f);     // Better than random
        EXPECT_LT(avg_loss, 500.0f);          // Reasonable loss
        EXPECT_FALSE(std::isnan(avg_loss));  // No NaN
        
        if (avg_loss < 10.0f) {
            std::cout << "✅ Training stable! Loss under control." << std::endl;
        } else {
            std::cout << "⚠️  Loss still high but stable (no explosion)" << std::endl;
        }
        
    } catch (const std::exception& e) {
        FAIL() << "Stable training failed: " << e.what();
    }
}